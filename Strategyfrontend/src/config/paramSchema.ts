...
export const PARAM_SCHEMA: Record<string, ParamSchemaItem> = {
  ...
  time_remaining_s: {
    ...
    visibility: "core",
    weight: "medium",
    label: { zh: "剩余时间阈值(秒)", en: "Time Remaining (s)" },
    desc: {
      zh: "只允许在市场剩余时间（秒）低于此阈值时入场，避免在市场初期不确定性过高时进场。1 tick ≈ 1 秒。",
      en: "Only allow entry when remaining time (seconds) is below this threshold, avoiding early entry when uncertainty is high. 1 tick ≈ 1 second.",
    },
    disable_value: 86400,
    disable_note: {
      zh: "设为 86400（24小时）：剩余时间几乎不会超过此值，入场时间限制实际失效，任意时刻均可入场",
      en: "Set to 86400 (24h): remaining time rarely exceeds this, time gate effectively disabled, entry allowed at any time",
    },
    type: "int",
    min: 0,
    max: 86400,
  ...
  btc_min_momentum: {
    ...
    desc: {
      zh: "启用BTC趋势过滤。回测前根据BTC价格的两段涨跌幅判断趋势方向和强度，|a1+a2| 必须大于此阈值才允许入场。值越大，仅允许更强的趋势通过。添加时自动引入两个检测窗口参数。",
      en: "Enable BTC trend filter. Compute BTC price trend from two time windows before the session. |a1+a2| must exceed this threshold to allow entry. Higher values filter out weak trends. Both trend windows are auto-included.",
    },
    type: "float",
    min: 0,
    max: 0.1,
    ...
    unit: "",
    scope: "unified",
    depends_on: ["btc_trend_window_1", "btc_trend_window_2"],
    default: 0.001,
  },
}
