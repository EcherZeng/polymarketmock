...
async def submit_optimization(req: OptimizeRequest):
    ...
    return {
        ...
        "estimated_total_runs": 1 * len(req.slugs) + req.max_rounds * req.runs_per_round * len(req.slugs),
    }
        ...
        for i, cfg in enumerate(r.configs):
            ...
            if cfg_digests:
                ...
                for mk in metric_keys:
                    ...
                    if not vals:
                        avg_metrics[mk] = 0
                    elif mk == "total_trades":
                        avg_metrics[mk] = sum(vals)
                    elif mk == "max_drawdown":
                        avg_metrics[mk] = round(max(vals), 4)
                    else:
